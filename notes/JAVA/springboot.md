...
(수정사항이 발생하면 결합된 모든 클래스를 수정해야 하기 때문이다.)
  ...
  생성자 주입이 가장 권장되는 방법인 이유   
   ...
   - final을 사용할 수 있다.    
    ...
    #### 8/4 추가
    컴포넌트 스캔 시 Repository, Service 등을 스캔하여 IoC 컨테이너에 bean들이 등록되게 된다.    
    이 때, Bean이 등록되는 방법은 기본 생성자를 호출하여 new를 하는 것이다.     
    그런데 기본 생성자가 아닌 다른 빈을 인자로 받는 생성자를 만들게 되면, 스프링이 IoC에 들어있는 빈을 주입시킨 다음 컨테이너에 등록하게 된다. 
    그러므로 생성자를 통한 주입이 가능한 것이다. 
    이 때, 생성자를 직접 만드는 것보다 주입이 필요한 bean에 final을 붙인다.
    그럼 해당 필드들은 초기화가 필요하게 되는데 RequiredArgsContructor를 붙이면 생성자를 자동으로 생성하여 초기화가 진행된다.   
    그러므로 생성자를 통한 초기화가 된다.
...
만약 컨트롤러 종료 전에 영속성 컨텍스트를 종료하면, ManyToOne 전략의 추가정보를 가져올 수 없다.    
이 때, Fetch type을 Lazy로 하게 되면 프록시 객체를 들고 오게 된다.   
만약 컨트롤러 단계에서 영속성 컨텍스트를 종료하지 않으면, 프록시 객체를 호출하여 추가정보를 들고올 수 있다.    
즉, 위의 과정과 같이 컨트롤러 종료 후 영속성 컨텍스트를 종료한다.   
프록시 객체를 호출하면 다시 jdbc 커넥션을 실행시키고 실제 데이터를 불러온 뒤 커넥션을 종료한다.    
설정의 open-in-view를 활성화시키면 lazy 로딩이 가능해진다. (false이면 컨트롤러 진입 직전에 영속성 컨텍스트도 종료된다.) 
